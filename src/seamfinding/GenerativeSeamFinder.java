...
public class GenerativeSeamFinder implements SeamFinder {
    ...
    public List<Integer> findHorizontal(Picture picture, EnergyFunction f) {
        ...
        seam = seam.subList(1, seam.size() - 1); // skip source then sink node
        List<Integer> result = new ArrayList<>(seam.size());
        for (Node node : seam) {
            // rest of nodes are pixels
    ...
    private static class PixelGraph implements Graph<Node> {
         ...
         */
        private static final Node sink = new Node() {
            @Override
            public List<Edge<Node>> neighbors(Picture picture, EnergyFunction f) {
                return List.of();
            }
         ...
         * @see EnergyFunction
         */
        public static class Pixel implements Node {
            private final int x;
            private final int y;
